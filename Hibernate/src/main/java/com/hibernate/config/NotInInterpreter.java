 ...
 * @author ZengYanyu
 * @Description
 * @Date 2020年11月22日 下午6:44:58
 * @see com.hibernate.config.NotInInterpreter.java
 */
public class NotInInterpreter implements Interpreter<InterpreteContext, NotInExpression> {
	/**
	 ...
	 */
	@Override
	public void interprete(InterpreteContext context, NotInExpression expression) {
		Object[] objs = expression.getValues();
		if (objs == null || objs.length == 0) {
			context.append(expression.getFullName());
			context.append(QueryConstants.BLANK);
			context.append(QueryConstants.NOT_IN);
